...
/**
 * REST controller for managing {@link gr.kavvada.warehouseerp.domain.Invoice}.
 */
@RestController
@RequestMapping("/api")
public class InvoiceResource {
     ...
     */
    @GetMapping("/invoices")
    public ResponseEntity<List<Invoice>> getAllInvoices(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
    ...
    /**
     * {@code GET  /invoices/payment-type/counts} : get all counts by Payment Type.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of PaymentTypeCountDTOs in body.
     */
    @GetMapping("/invoices/payment-type/counts")
    public ResponseEntity<List<PaymentTypeCountDTO>> getPaymentTypeCounts() {
        log.debug("REST request to get a page of Invoices");
        List<PaymentTypeCountDTO> paymentTypeCounts = invoiceService.findPaymentTypeCounts();
        return ResponseEntity.ok().body(paymentTypeCounts);
    }
}
